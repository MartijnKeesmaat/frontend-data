...
var addLabelsToBarChart = function addLabelsToBarChart(svg, categories, labelWidth, barSpacing) {
  svg.append('g').attr('class', 'bar-text').selectAll('text').data(categories).enter().append('text').text(function (d) {
    return (0, _helpers.capitalize)(d.name);
  }).attr('y', function (d, i) {
    return i * barSpacing + 10;
  }).attr('class', 'label').attr('dy', 0).attr('text-anchor', 'end').attr('transform', 'translate(95,' + 0 + ')').call(_helpers.wrap, labelWidth);
};
...
var addGlobalSVGBarChart = function addGlobalSVGBarChart(width, height) {
  return d3.select('.bar-chart-container').insert('svg').attr('width', width).attr('height', height);
...
function addActiveClassToBar(index, color) {
...
function renderBarChart(categories, width, height) {
  ...
  (0, _barFunctions.addGridlinesToBarChart)(svg, width, height, xScale); // Render donut

  updateDonutChart(getCurrentDonutData(0, categories), donutContainer, pie, donutConfig.colors, arc, categories, xScale);
  updateDonutChart(getCurrentDonutData(0, categories), donutContainer, pie, donutConfig.colors, arc, categories, xScale); // also stores event for donut chart
...
function updateDonutChart(data, donutContainer, pie, color, arc, categories, xScale) {
  var slice = donutContainer.select('.slices').selectAll('path.slice').data(pie(data)).on('mouseover', function (d, i) {
    ...
    d3.select('.donut-title').text(d.data.value);
    (0, _barFunctions.addActiveClassToBar)(getSelectedBar(), '#fba16c');
...
var addBarsToBarChart = function addBarsToBarChart(xScale, svg, categories, barheight, barSpacing, donutContainer, pie, colors, arc) {
  svg.append('g').attr('class', 'bar-group').selectAll('rect').exit().remove().data(categories).enter().append('rect').attr('x', function (d, i) {
    return 97;
  }).attr('y', function (d, i) {
    return i * barSpacing;
  }).attr('width', function (d) {
    ...
    d3.selectAll('.d-bar').filter(function (d, j) {
      return j === i;
    }).attr('fill', '#fba16c');
  ...
  (0, _barFunctions.addActiveClassToBar)(0, '#fba16c'); // add active class to first item
...
function brusher() {
  var brush = document.querySelector('#brusher');
  var h = 855 - 280; // height inner-container - height svg
...
function brusherBars(categories) {
  var svgH = 300;
  var barH = 6;
  var xScale = (0, _barFunctions.addXScaleBarChart)(92, 15, categories);
  var container = d3.select('.d-bars').append('svg').attr('transform', 'translate(0,' + -10 + ')');
  container.selectAll('rect').data(categories).enter().append('rect').attr('class', 'd-bar').attr('x', function (d, i) {
    return 0;
  }).attr('y', function (d, i) {
    return i * (svgH + 40) / 19;
  }).attr('width', function (d) {
  ...
  d3.selectAll('.d-bar').filter(function (d, i) {
    return i === 0;
  }).attr('fill', '#fba16c');
}
...
function renderCharts(categories) {
  var dataForFP = categories.slice(0, 16);
